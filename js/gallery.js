...
lightbox.addEventListener('touchstart', (e) => { startX = e.touches[0].clientX; currentEl = lightboxContent.firstElementChild; if (currentEl) currentEl.style.transition = 'none'; });
lightbox.addEventListener('touchmove', (e) => { if (!currentEl) return; const deltaX = e.touches[0].clientX - startX; currentEl.style.transform = `translateX(${deltaX}px)`; });
lightbox.addEventListener('touchend', (e) => { if (!currentEl) return; const deltaX = e.changedTouches[0].clientX - startX; currentEl.style.transition = 'transform 0.3s ease'; if (deltaX > 50) { currentEl.style.transform = 'translateX(100%)'; currentEl.addEventListener('transitionend', showPrev, { once: true }); } else if (deltaX < -50) { currentEl.style.transform = 'translateX(-100%)'; currentEl.addEventListener('transitionend', showNext, { once: true }); } else { currentEl.style.transform = 'translateX(0)'; } currentEl = null; });